...
if (typeof window !== "undefined") {
    ...
    // This stops GSAP from lagging and recalculating when the mobile browser's address bar hides/shows
...
export default function HomePage() {
    ...
    useGSAP(() => {
        ...
        const timer = setTimeout(() => {
            ...
            // =========================================================
            // 4. GSAP MATCHMEDIA (RESPONSIVE GSAP PINNING)
            // =========================================================
            const mm = gsap.matchMedia();

            // --- MOBILE ONLY ANIMATIONS ---
            mm.add("(max-width: 767px)", () => {
                const bTrack = bentoTrackRef.current;
                const bSection = bentoContainerRef.current;

                if (bTrack && bSection) {
                    const getScrollAmount = () => -(bTrack.scrollWidth - window.innerWidth);
                    const tween = gsap.to(bTrack, { x: getScrollAmount, ease: "none" });

                    ScrollTrigger.create({
                        trigger: bSection,
                        start: "top top",
                        end: () => `+=${bTrack.scrollWidth - window.innerWidth}`,
                        pin: true,
                        animation: tween,
                        // Changed scrub: 1 to scrub: true to lock perfectly to your finger
                        scrub: true,
                        // Tells GSAP to prepare the pin instantly to prevent stutter
                        anticipatePin: 1,
            ...
            // 7. BEST TOURS - HORIZONTAL SCROLL (Universal)
            ...
            if (track && section) {
                ...
                ScrollTrigger.create({
                    ...
                    pin: true,
                    animation: tween,
                    // Changed to true for tighter mobile connection
                    scrub: true,
                    anticipatePin: 1,
                    invalidateOnRefresh: true,
                });
    ...
    return (
        <div ref={containerRef} className="relative w-full overflow-hidden bg-background">
            ...
            {/* HERO SECTION */}
            <section
                className="relative h-[100svh] flex flex-col justify-center items-center text-center px-4 overflow-hidden">
            ...
            <section className="py-16 md:py-24 bg-background overflow-hidden">
                ...
                <div className="relative flex flex-col gap-4 overflow-hidden w-full">
                    ...
                    <div className="memory-track-1 flex gap-4 w-max">
                    ...
                    <div className="memory-track-2 flex gap-4 w-max -ml-20">
            ...
            {/* GSAP RESTORED BENTO GRID */}
            <section ref={bentoContainerRef} className="relative h-[100svh] md:h-auto md:py-32 w-full bg-background overflow-hidden flex flex-col justify-center md:block">
                <div className="absolute top-24 left-4 md:static text-center md:mb-20 z-10 pointer-events-none md:pointer-events-auto">
                ...
                {/* Mobile is a flex track, Desktop is the strict grid */}
                <div ref={bentoTrackRef} className="flex flex-nowrap items-center md:grid md:grid-cols-4 md:grid-rows-2 gap-4 h-[50vh] md:h-[600px] w-max md:w-full px-4 md:px-8 max-w-7xl mx-auto pt-20 md:pt-0 pr-[15vw] md:pr-8">
                    <Link href="/tours" className="bento-reveal w-[85vw] md:w-auto shrink-0 group relative md:col-span-2 md:row-span-2 rounded-3xl overflow-hidden h-full block border border-white/5">
                        <Image src="/images/mountain.webp" alt="Mountain" fill sizes="(max-width: 768px) 100vw, 50vw" className="object-cover transition-transform duration-700 group-hover:scale-110" />
                        <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/20 to-transparent"></div>
                        <div className="absolute bottom-6 left-6"><h3 className="text-3xl md:text-4xl font-black text-white group-hover:text-primary transition-colors">Mountain</h3></div>
                    </Link>
                    <Link href="/tours" className="bento-reveal w-[85vw] md:w-auto shrink-0 group relative md:col-span-2 md:row-span-1 rounded-3xl overflow-hidden h-full block border border-white/5">
                        <Image src="/images/jungle.webp" alt="Jungle" fill sizes="(max-width: 768px) 100vw, 50vw" className="object-cover transition-transform duration-700 group-hover:scale-110" />
                        <div className="absolute inset-0 bg-gradient-to-t from-black/80 to-transparent"></div>
                        <div className="absolute bottom-6 left-6"><h3 className="text-2xl font-black text-white group-hover:text-primary transition-colors">Jungle</h3></div>
                    </Link>
                    <Link href="/tours" className="bento-reveal w-[85vw] md:w-auto shrink-0 group relative md:col-span-1 md:row-span-1 rounded-3xl overflow-hidden h-full block border border-white/5">
                        <Image src="/images/ocean.webp" alt="Ocean" fill sizes="(max-width: 768px) 100vw, 25vw" className="object-cover transition-transform duration-700 group-hover:scale-110" />
                        <div className="absolute inset-0 bg-gradient-to-t from-black/80 to-transparent"></div>
                        <div className="absolute bottom-6 left-6"><h3 className="text-xl font-black text-white group-hover:text-primary transition-colors">Ocean</h3></div>
                    </Link>
                    <Link href="/tours" className="bento-reveal w-[85vw] md:w-auto shrink-0 group relative md:col-span-1 md:row-span-1 rounded-3xl overflow-hidden h-full block border border-white/5">
                        <Image src="/images/desert.webp" alt="Desert" fill sizes="(max-width: 768px) 100vw, 25vw" className="object-cover transition-transform duration-700 group-hover:scale-110" />
                        <div className="absolute inset-0 bg-gradient-to-t from-black/80 to-transparent"></div>
                        <div className="absolute bottom-6 left-6"><h3 className="text-xl font-black text-white group-hover:text-primary transition-colors">Desert</h3></div>
                    </Link>
                </div>
            </section>
            ...
            {/* BEST TOURS HORIZONTAL SCROLL (RESTORED GSAP) */}
            <section ref={bestToursRef} className="relative h-screen w-full bg-background overflow-hidden flex flex-col justify-center border-t border-white/5">
                ...
                <div ref={bestToursTrackRef} className="flex flex-nowrap items-center gap-6 md:gap-8 px-4 md:px-20 w-max pr-[15vw] md:pr-[25vw] pt-20">
                    {featuredTours.map((tour) => (
                        <Link href={`/tours/${tour.id}`} key={tour.id} className="block w-[85vw] md:w-[45vw] lg:w-[35vw] shrink-0 group relative h-[50vh] md:h-[65vh] rounded-3xl overflow-hidden cursor-pointer shadow-2xl border border-white/5">
            ...
            <section className="py-32 relative px-4 flex justify-center text-center bg-background border-t border-white/5">
                <div className="relative z-10 max-w-3xl">
                    ...
                    <Link href="/contact" className="inline-flex items-center justify-center gap-3 bg-white text-black hover:bg-primary hover:text-white px-10 py-5 rounded-full font-black text-lg transition-all hover:scale-105 shadow-xl hover:shadow-[0_0_30px_var(--color-primary)]">
                        Contact Us Today
                    </Link>
                </div>
            </section>
